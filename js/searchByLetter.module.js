export class SearchByLetter {
    constructor(){
        this.letter = document.getElementById('searchLetter').addEventListener('input' , (event)=> {
            const searchQuery = event.target.value;
            this.getSearchByLetter(searchQuery);
        });
    }

    async getSearchByLetter(letter){
        if (letter === '') {
            document.getElementById('searchPage').innerHTML = '';
            return;
        }
        const searchByLetterResponse = await fetch(`https://www.themealdb.com/api/json/v1/1/search.php?f=${letter}`);
        const searchByLetterData = await searchByLetterResponse.json();
        this.displaySearchByLetter(searchByLetterData);
    ...
    displaySearchByLetter(searchByLetterData){
        let searchByLetterCards = ``;
        for (let i = 0; i < searchByLetterData.meals.length ; i++) {
            searchByLetterCards += `
            <div class="meal" data-id="${searchByLetterData.meals[i].idMeal}">
                <img src="${searchByLetterData.meals[i].strMealThumb}" alt="image">
                <div class="layer"><p>${searchByLetterData.meals[i].strMeal}</p></div>
            </div>`;
        }
        document.getElementById('searchPage').innerHTML = searchByLetterCards;
    }
